...
import { useSessionDueCards } from "@/hooks/useSession";
...
export default function SessionStudyPage() {
  ...
  const { data: cards, isLoading, isError, refetch } = useSessionDueCards(sessionId);
  ...
  const reviewMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: sessionKeys.study(sessionId) });
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    if (rating) reviewMutation.mutate({ cardId: cards[currentIndex].id, rating: rating.value });
  ...
  if (isError) {
    return (
      <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center gap-4 text-white">
        <p role="alert">Could not load this study session.</p>
        <button
          onClick={() => refetch()}
  ...
  if (sessionDone || !cards || cards.length === 0) {
    const reviewedCount = cards?.length ?? 0;
    return (
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex flex-col">
      <header className="px-6 py-4 flex items-center justify-between border-b border-gray-800">
        ...
        <span className="text-gray-400 text-sm">{currentIndex + 1} / {cards.length}</span>
      </header>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
        ...
        >
          ...
          >
            ...
            <div style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }} className="absolute inset-0 flex items-center justify-center">
        ...
        {!isFlipped && (
          <p className="mt-6 text-gray-500 text-sm">
            Click or press <kbd className="bg-gray-800 px-2 py-0.5 rounded text-xs">Space</kbd> to reveal
          </p>
        )}
        ...
        {isFlipped && (
          <div className="mt-8 flex gap-3 w-full max-w-2xl">
            {RATINGS.map((rating) => (
              <button
                key={rating.value}
                onClick={() => reviewMutation.mutate({ cardId: card.id, rating: rating.value })}
